...
function printSeries() {
    ...
    for (let i = 0; i < model.series.length; i++) {
       ...
       ${Netflix = model.series[i].Netflix != '' ? `<input class="btn" type="image" src="/logos/Netflix-Logo.png" width="96" height="54" onclick="openInNewTab('${model.series[i].Netflix}');">` : ''}
       ${hbo = model.series[i].hbo != '' ? `<input class="btn" type="image" src="/logos/HBO_logo_blue.png" width="86" height="44" onclick="openInNewTab('${model.series[i].hbo}');">` : ''}
       ${viaplay = model.series[i].viaplay != '' ? `<input class="btn" type="image" src="/logos/viaplay-logo-1-min.png" width="110" height="48" onclick="openInNewTab('${model.series[i].viaplay}');">` : ''}
       ${youtube = model.series[i].youtube != '' ? `<input class="btn" type="image" src="/logos/red-youtube-logo-png-xl.png" width="64" height="54" onclick="openInNewTab('${model.series[i].youtube}');">` : ''}
       ${googleplay = model.series[i].googleplay != '' ? `<input class="btn" type="image" src="/logos/Google_Play_logo_store.png" width="54" height="54" onclick="openInNewTab('${model.series[i].googleplay}');">` : ''}
       ${appleTV = model.series[i].appletv != '' ? `<input class="btn" type="image" src="/logos/baa.png" width="84" height="54" onclick="openInNewTab('${model.series[i].appletv}');">` : ''}
       ${amazon = model.series[i].amazon != '' ? `<input class="btn" type="image" src="/logos/prime-video-amazon.webp" width="120" height="54" onclick="openInNewTab('${model.series[i].amazon}');">` : ''}
       ${tv2 = model.series[i].tv2 != '' ? `<input class="btn" type="image" src="/logos/TV_2_Norge.webp" width="44" height="54" onclick="openInNewTab('${model.series[i].tv2}');">` : ''}
       ${disney = model.series[i].disney != '' ? `<input class="btn" type="image" src="/logos/disneyplus.png" width="100" height="54" onclick="openInNewTab('${model.series[i].disney}');">` : ''}
       ${discovery = model.series[i].discovery != '' ? `<input class="btn" type="image" src="/logos/dicovery.png" width="94" height="69" onclick="openInNewTab('${model.series[i].discovery}');">` : ''}
       </center>
       </div>
       </div>
       `
